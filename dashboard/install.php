          ...
          <?php include("includes/navbar.php");?>
                ...
                <div class="board-canvas">
                   ...
                   <div class="step-head row">
                     <?php
                        $r=mysql_query("select * from ins_heading"); $r1=mysql_num_rows($r); $col2=100/$r1; $css2=($r1*10)+50;
...
$query1 ="SELECT * FROM ins_heading";
...
{ 
                           if($proj_res['ins_step']==$heading_res['id'])
                           {

?>                       <div class="project" value="<?php echo $heading_res['id']; ?>" data-assignedto="<?php echo $proj_res['assigned_to']; ?>" data-desc="<?php echo $proj_res['description']; ?>" data-idate="<?php echo $proj_res['initial_date']; ?>" data-reason="<?php echo $proj_res['reason']; ?>"
                              
                              data-edate="<?php echo $proj_res['last_date']; ?>" data-sales="<?php echo $proj_res['sales_rep']; ?>" data-type="<?php echo $proj_res['product_type']; ?>" data-bid="<?php echo $proj_res['product_bid']; ?>" data-id="<?php echo $proj_res['id']; ?>" data-step="<?php echo $heading_res['name']; ?>" data-duedate="<?php echo $proj_res['due_date']; ?>" >
                         ...
                         <div class="proj-head">
                            ...
                            /* $i=$proj_res['id']; $newdate="";
                                $qqq="select * from boxes_info where type=4 and user=$id and project=$i order by id2 desc";
                                $ress=mysql_query($qqq);
                                $resss=mysql_fetch_array($ress);
                            if(mysql_num_rows($ress)>0)
                            {
                            $ideee=$resss['id2'];
                           $qqq2="select * from admin_data where typee=4 and user_id=$id and project_id=$i and id2>$ideee";
...
<script src="<?php echo $unc;?>js/installation.js"></script>
<?php
