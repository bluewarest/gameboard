...
$ck=$_POST['checklist'];
$where=$_POST['where'];
$q="insert into ins_checklists (name,wheree) values ('$ck','$where')";
$res=mysql_query($q);
if($res)
...
{
$h=$_POST['heading'];
$q="insert into ins_heading (name) values ('$h')";
$res=mysql_query($q);
if($res)
    echo "Added sucessfully";
else
...
?>
            ...
            <h3>Add Check List</h3>
                <form action="" method="POST">
                    ...
                    <select name="where">
                        <?php
                        $res=mysql_query("select * from ins_heading");
...
?>
         ...
         <!--########################################################################################################-->
            ...
            { $n=$data['name']; $co=0; $color="";
             //************color work***************//
             $c="select * from ins_checklists where wheree='$n'";
             $cr=mysql_query($c); $f=mysql_num_rows($cr);
             while($cd=mysql_fetch_array($cr))
                 {  $ideee=$cd['id'];
                    $kc="select * from boxes_info where type=5 and project=$pid and id2=$ideee";
            ...
            <div class="accordion-section">
				<a style="background:<?php echo $color; ?>;" class="accordion-section-title" href="#accordion-<?php echo $data['id']; ?>">
                    ...
                    {
                    ?>
                    <span onclick="dell(<?php echo $data['id']; ?>,'ins_heading','heading')" style="">&nbsp;X&nbsp;</span>
                    <?php } ?>
                    <?php echo $data['name']; ?>
                  <!--=============================================================================-->
                   <?php
                $n=$data['name'];  $td=date('d'); $td=$td+4; $d=date('Y-m-'); $d=$d.$td; $num_ck=0;  $i=$pid; $r_ids=""; $cp=0; $newdate="";
                $one="select * from ins_checklists where wheree='$n'";
                $one_q=mysql_query($one); $one_r=mysql_num_rows($one_q);
                    ...
                    { $one_id=$one_f['id'];
                     $two="select * from admin_data where typee=5 and user_id=$id and project_id=$i and id2=$one_id and due_date<'$d'";
                     $two_q=mysql_query($two); $two_r=mysql_num_rows($two_q); $two_f=mysql_fetch_array($two_q);
                        if($two_r>0)
                        {
                            $three="select * from boxes_info where type=5 and user=$id and project=$i and id2=$one_id";
				...
				<div id="accordion-<?php echo $data['id']; ?>" class="accordion-section-content">
                        <div class="col-md-12">
                        <?php
                        $q2="select * from ins_checklists where wheree='$n'";
                        $res2=mysql_query($q2);
                        while($data2=mysql_fetch_array($res2))
                         {  $idee=$data2['id'];
                            $k="select * from boxes_info where type=5 and project=$pid and id2=$idee"; $res3=mysql_query($k); $nums=mysql_num_rows($res3); $arr=mysql_fetch_array($res3);

                        //********************//
                         $k2="select * from additional_email where typee=5 and id2=$idee";
                ...
                <label><label style="display:inline" onclick='show("<?php echo $data2['id']; ?>")'><?php echo $data2['name']; if (strpos($r_ids,$data2['id']) !== false) { echo "<b> *</b>"; } ?>&nbsp;&nbsp;(Default Email: <?php echo $ae; ?> )</label>
                    ...
                    {
                    ?>
                    &nbsp;&nbsp;<span onclick="dell(<?php echo $data2['id']; ?>,'ins_checklists','checklist')" style="background: red;color:white;">&nbsp;X&nbsp;</span>
                    <?php } ?>
                    </label>
                        <?php
                        }
...
{  var a,b,c=""; var ne="<p style='display:none;'><input type='checkbox' name='default' id='default'/>Send default Email?</p>"; var at=""; var content2="";
    ...
    $.ajax({
            ...
            data: { data : data, type:'5',pid:pid ,user_id:id },
...
function usave(data)
{
   ...
   { show(data);
            ...
            data: { action:'add' , data:data, type:'5',user_id:<?php echo $id; ?>,pid:<?php echo $pid; ?> },
            success: function (data) {
   ...
   else
    {
            ...
            type: 'post',
            url: 'admin/user-save-info.php',
            data: { action:'remove' , data:data, type:'5' ,user_id:<?php echo $id; ?>,pid:<?php echo $pid; ?>},
            success: function (data) {
              if(data=="info saved"){}else{ alert(data);}
            }
            });
