...
function display_event_by_oid($oid)
{
	...
	if(!empty($event->mtime))
		$event_header->add(tag('div', __('Last modified at: '),
				...
				$cdate=time($event->get_datetime_string());
	$menu_tag = tag('div', attrs('class="phpc-bar ui-widget-content"')); 
	// Add modify/delete links if this user has access to this event.
   // $ev=var_dump($event);
    if(!($event->start_hour)){
    $event->start_hour='00';$event->end_hour='00';
    }
      if(!($event->start_minute)){
    $event->start_minute='00';$event->end_minute='00';
    }
    $startdate=$event->start_year.'0'.$event->start_month.$event->start_day.'T'.$event->start_hour.$event->start_minute.'00Z';
    $enddate=$event->end_year.'0'.$event->end_month.$event->end_day.'T'.$event->end_hour.$event->end_minute.'00Z';
        if($event->can_modify()) {
		$menu_tag->add(array(create_event_link(__('Modify'),
					...
					"\n", "\n",
					'<a style="color:red" target="_blank" href="http://www.google.com/calendar/event?action=TEMPLATE&text='.$event->get_subject().'&dates='.$startdate.'/'.$enddate.'&details='.$event->get_desc().'"&location=neverbluecrm>Sync with Google</a>'.$ev,
					"\n"));
	}
