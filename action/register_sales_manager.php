		...
		if(preg_match($validEmailString,$email)):
			...
			$sql1 = mysql_query("SELECT email FROM users WHERE email = '".$email."'");
				...
				$sql2 = mysql_query("SELECT FROM users WHERE username = '".$username."'");
					...
					$firstname = addslashes(ucfirst(strtolower(stripslashes(strip_tags(trim($_SESSION['REGISTER_POST']['firstname']))))));
					$firstname = mysql_real_escape_string($firstname);
					$lastname = addslashes(ucfirst(strtolower(stripslashes(strip_tags(trim($_SESSION['REGISTER_POST']['lastname']))))));
					...
					$position = stripslashes(strip_tags(trim($_SESSION['REGISTER_POST']['position'])));
                    ...
                    $assigned_to = stripslashes(strip_tags(trim($_SESSION['REGISTER_POST']['assigned_to'])));
					...
					$sql = mysql_query("INSERT INTO registration (username,email,firstname,lastname,phone,company,position,address,city,state,zipcode,country,registration_key,assigned_to) VALUES ('".$username."','".$email."','".$firstname."','".$lastname."','".$phone."','".$company."','".$position."','".$address."','".$city."','".$state."','".$zipcode."','".$country."','".$registration_key."','".$assigned_to."')");
						if($sql):
...
$message2.='<br />http://crm.spm-sites.com/?c='.$registration_key;
					    mail("raidthelibra@gmail.com,jimmy@express-business-solutions.com,josh@thestrategicpoint.com, monicakilian1@me.com,usmanpervez1992@gmail.com,jbornamann@gmail.com", $subject2, $message2, implode("\r\n",$headers2));	
					    
					    $_SESSION['REGISTER'] = true;
					    unset($_SESSION['REGISTER_POST']);
					    header("location:../registration");			
						else:
						$_SESSION['REGISTER_ERROR'] =  'Registration: Aready Exists for Approval, Check your Email';
						header("location:../");
						endif;
					endif;
				endif;
